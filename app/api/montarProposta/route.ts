...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { razaoSocial, cnpj, responsavel, objeto, valor, observacoes, conteudoEdital } = data;
...
A proposta deve ser formal, clara e usar termos objetivos. Inclua parágrafos que reforcem a experiência da empresa e compromisso com prazos e conformidade.`;
    ...
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: "Você é um especialista em elaboração de propostas técnicas e comerciais para licitações públicas no Brasil."
        },
        {
          role: "user",
          content: prompt
