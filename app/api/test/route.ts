...
export async function GET(request: NextRequest) {
  const update = await openai.beta.assistants.update(
    "asst_l08DvGpHDAUTRQsdqzmpUDG1",
    {
      instructions: `
      DADOS DA EMPRESA (contexto para análise):  
      - companyName: Escola START  
      - cnpj: 46.558.310/0001-27  
      - productServiceDescription: Educação em geral  
  ...
  const retrieve = await openai.beta.assistants.retrieve(
    "asst_l08DvGpHDAUTRQsdqzmpUDG1"
  );

  return NextResponse.json(retrieve);
